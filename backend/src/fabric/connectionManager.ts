...
export class ConnectionManager {
    ...
    private async loadCredentialsFromYaml(yamlPath: string): Promise<UserCredentials> {
    ...
    private async loadTLSCACert(mspId: string): Promise<Buffer> {
        ...
        const networkYamlPath = path.join(process.env.FABRIC_RESOURCES_PATH!, 'network.yaml');
        ...
        if (!peerConfig || !peerConfig.tlsCACerts?.pem) {
            throw new Error(`No se encontró el certificado TLS CA para el peer ${peerName}`);
        }

        return Buffer.from(peerConfig.tlsCACerts.pem);
    }

    async connectGateway(user: User): Promise<{ gateway: Gateway, client: grpc.Client }> {
        ...
        const client = new grpc.Client(
            ...
            grpcCredentials,
            {
                'grpc.ssl_target_name_override': user.organization.peerEndpoint.split(':')[0],
    ...
    queryTransaction(gateway: Gateway, client: grpc.Client, functionName: string, ...args: string[]): Promise<Uint8Array> {
