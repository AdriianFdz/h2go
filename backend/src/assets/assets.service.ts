...
export class AssetsService {
    ...
    async registerProduction(user: User, registerProductionDTO: RegisterProductionDto) {       
        if (registerProductionDTO.assetType === AssetType.ELECTRICITY && user.organization.id !== process.env.REE_ORG_ID) {
            throw new ForbiddenException('Only REE organization can register ELECTRICITY production.');
        }
        
        if (registerProductionDTO.assetType === AssetType.H2 && user.organization.id !== process.env.ENAGAS_GTS_ORG_ID) {
            throw new ForbiddenException('Only ENAGAS GTS organization can register H2 production.');
        }
